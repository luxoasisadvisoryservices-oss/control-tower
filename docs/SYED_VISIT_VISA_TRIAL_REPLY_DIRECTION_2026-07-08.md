# Syed Visit Visa / Trial Reply Direction - 2026-07-08

Status: internal draft for Damiano. Not sent.
Business: Lux Oasis Holiday Homes LLC.
Person: Syed Imran.

## Context

Syed replied that he will arrive in Dubai on Saturday morning, 11 July 2026. He already has a 60-day visit visa and prefers to use that period. He understands the compliance concern and suggested remote work while both sides evaluate fit.

He also asked for clarity before starting:

- working setup because there is no office;
- access to portals such as Dubizzle, Bayut and Property Finder, or other lead sources;
- inventory and marketing resources;
- SIM card;
- reasonable travel expenses for meetings and property visits;
- an offer letter outlining the trial arrangement and expectations;
- option to meet in Dubai on Saturday after arrival, or continue by call.

## Damiano Direction

Damiano will travel on Friday, 10 July 2026, and will be in Italy, so an in-person Saturday meeting may not work.

Reply direction:

- thank Syed for the clear and transparent message;
- say Damiano is travelling on Friday and will be in Italy;
- say Damiano will send a structured proposal / trial outline instead of rushing an informal arrangement;
- confirm Lux Oasis is preparing the practical tools needed, including company portal accounts for Property Finder and Dubizzle;
- mention Bayut can also be considered if it fits the plan and budget;
- keep compliance tight: no promise that he can work on a visit visa until the correct route is confirmed;
- frame the first stage as a clear 60-day proof arrangement, subject to the legal route and written scope;
- say the proposal will cover role, expected lead targets, reporting, tools, expenses, commercial structure and what happens after the trial.

## Safe Reply Draft

```text
Hi Syed,

Thank you for the clear and honest message. I appreciate you laying everything out properly before you arrive.

I am travelling on Friday and will be in Italy, so meeting in person on Saturday may be difficult. Rather than rush the arrangement, I will send you a clear proposal outlining what I have in mind for the trial, the expectations from both sides, and the tools/resources we can provide.

I agree with you that if we test this properly, we need the right setup. We are already looking at creating company accounts for Property Finder and Dubizzle under Lux Oasis, and we can also review Bayut or other lead sources depending on the plan. The proposal will also cover inventory, marketing material, communication setup, reporting, and reasonable travel/meeting expenses where approved.

On the visa/compliance side, I want to keep everything clean. Since you are arriving on a 60-day visit visa, we need to be careful about what is allowed before any work-permit or compliant arrangement is confirmed. I do not want either side to start in a way that creates issues later.

My idea is to structure this first as a 60-day proof period, with clear targets around owner/agent/property sourcing, weekly pipeline reporting, and a clear review at the end. If the pipeline and working relationship are strong, we can then discuss the next step properly.

I will send you the proposal idea so you can review it, and then we can discuss it by call while I am away.
```

## Proposal Outline To Prepare

1. Role
   - Development / acquisition partner for Lux Oasis Holiday Homes.
   - Focus: owners, landlords, agents, investors, developers and suitable apartments.

2. First-stage scope
   - 60-day proof period.
   - Work only within the compliant legal route confirmed for his visa/status.
   - No binding promises to owners, agents, landlords or guests without Lux Oasis approval.

3. Targets
   - 5-10 real owner/agent/investor routes actively worked.
   - 1-2 units moved to serious interest, economics, viewing, document or contract-discussion stage.
   - Weekly pipeline report.

4. Tools/resources
   - Company portal accounts to be explored/created for Property Finder and Dubizzle.
   - Bayut or other sources to be assessed.
   - Approved inventory and marketing material.
   - Communication setup to be confirmed.
   - SIM/travel expenses only if approved and tied to real meetings/property visits.

5. Commercial structure
   - No fixed long-term employment promise before proof.
   - Commercial upside should be tied to Lux Oasis retained net profit from units he personally brings and helps close/operate.
   - Working range remains 15%, 20% or 25% depending on contribution and property economics, subject to final approval/legal review.

6. Compliance
   - Do not treat a visit visa as permission to work.
   - Confirm temporary, part-time, flexible, freelance or referral-only route before operational work starts.
   - Offer letter or agreement should be written only after the legal route is clear.

## Control Rule

Do not send this externally without Damiano approval.
